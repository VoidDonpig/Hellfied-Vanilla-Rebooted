...
# Remove zombie modifier
    ...
    execute if entity @s[tag=hvr.mob_asset.zombie.boosting] run function hvr:asset/mob/zombie/boost_buff/reset
    execute if entity @s[tag=hvr.mob_asset.zombie.boosting_rebound] run function hvr:asset/mob/zombie/boost_buff/refresh_rebound

# Equipment
    execute unless data storage hvr:asset/mob {is_newly_converted_mob:true} if score $difficulty hvr.global matches 1 run function hvr:asset/mob/drowned/equipment/1
