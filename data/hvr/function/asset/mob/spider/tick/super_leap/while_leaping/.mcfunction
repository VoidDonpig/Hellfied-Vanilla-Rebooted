...
# When near target
    execute if entity @s[tag=!hvr.asset.mob.spider.leaping.motion_canceled] if entity @n[tag=hvr.target,distance=..16] run function hvr:asset/mob/spider/tick/super_leap/while_leaping/when_near_target/check